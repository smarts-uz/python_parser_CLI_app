...
from django_orm.db.db_functions import update_target_group, update_byte_group
...
def copy_file_with_custom_date(src, dst, custom_date,group_id,file_name):
    ...
    try:
        ...
        byte = os.path.getsize(filename=file_dst)
        update_byte_group(pk=group_id,byte=byte)
    # Set the custom date
